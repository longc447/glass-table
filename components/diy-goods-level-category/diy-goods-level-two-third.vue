<template>
	<view class="category-level" :style="mainHeight">
				...
				<view class="twoScroll" @scrolltolower="getGoodsList()">
					<view class="category-cate-top">
						<scroll-view scroll-x="true" class="three-two-cate" v-if="twoCateList.length">
							<text class="three-two-cate-item using-hidden"
							...
							<text class="three-two-cate-item using-hidden"
								...
								v-for="(item, index) in twoCateList" :key="index"
					...
					<scroll-view scroll-y="true" :style="{ height: 'calc(100% - ' + height + ' )' }"
						...
						<image class="cate-adv" v-if="categoryAdvImage"
							@error="categoryAdvImage = $util.img('/upload/uniapp/default_ad.png')"
							:src="$util.img(categoryAdvImage)" mode="aspectFill"></image>
						<view class="twoScroll-item-box" v-for="(item, index) in goodsList" :key="index"
							@click="toDetail(item.sku_id)">
							<image class="twoScroll-item-image" :src="$util.img(item.sku_image, { size: 'mid' })"
								@error="goodsImgError(index)" mode="widthFix" />
							...
							<view class="goods_detail">
								...
								<view class="goods_price">
									...
									<text class="line-price">
										<text class="price-icon">￥</text>
										{{item.is_wholesaler == 3 ? item.price_pf : 
											item.market_price > 0 ? item.market_price : item.price}}
										<!-- {{ item.market_price > 0 ? item.market_price : item.price }} -->
									</text>
									<view :class="item.goods_spec_format ? 'careBox_more' : 'careBox'"
										v-if="item.is_virtual == 0" @click.stop>
										<text v-if="item.goods_spec_format" @click.stop="getGoodsSkuDetail(item.sku_id)"
											class="color-base-bg">选规格</text>
										<block v-else>
											<text class="iconfont iconjianshao color-base-text" v-if="item.num > 0"
												@click.stop="cartNumChange('minus', item)"></text>
											<input :value="item.num" readonly disabled="disabled" unselectable="on"
												v-if="item.num > 0" @click.stop />
											<text class="iconfont iconadd-fill color-base-text"
												@click.stop="cartNumChange('add', item)"></text>
										</block>
									</view>
								</view>
							</view>
						...
						<ns-empty v-if="isAll && goodsList.length == 0 && twoCateList.length" :isIndex="!1" :fixed="!1"
							text="该分类下暂无商品"></ns-empty>
					</scroll-view>
				</view>
			</view>
		</view>
		...
		<!-- 加载中 -->
		<view class="mescroll-upwarp" v-show="!isLoadingCate">
			<ns-loading></ns-loading>
		</view>

		<ns-goods-sku-category ref="goodSkuNew" :goods-detail="goodsSkuDetail" @refresh="refreshGoodsSkuDetail">
		</ns-goods-sku-category>
