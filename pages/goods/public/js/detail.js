...
export default {
  ...
  methods: {
    ...
    async getGoodsSkuDetail(skuId) {
      ...
      let data = res.data;
      if (data.goods_sku_detail == null) return this.$util.redirectTo('/pages/index/index/index', {}, 'reLaunch');
      ...
      // 当前商品SKU规格
      if (this.goodsSkuDetail.sku_spec_format) this.goodsSkuDetail.sku_spec_format = JSON.parse(this.goodsSkuDetail
        .sku_spec_format);
      ...
      if (this.goodsSkuDetail.goods_attr_format) {
        ...
        for (var i = 0; i < this.goodsSkuDetail.goods_attr_format?.length; i++) {
          for (var j = 0; j < goods_attr_format?.length; j++) {
            if (this.goodsSkuDetail.goods_attr_format[i].attr_id == goods_attr_format[j].attr_id && this
      ...
      if (this.goodsSkuDetail.promotion_type == 1 && this.addonIsExit.discount) {
        ...
        if ((this.goodsSkuDetail.end_time - res.timestamp) > 0) {
          this.goodsSkuDetail.discountTimeMachine = this.$util.countDown(this.goodsSkuDetail.end_time - res
            .timestamp);
        } else {
    ...
    buyNowWholesaler() {
      ...
      this.$util.redirectTo('/pages/login/register/wholesale/wholesale');
    },
    swiperChange(e) {
      ...
      this.$api.sendRequest({
        ...
        success: res => {
          ...
          if (data) {
            ...
            if (this.goodsEvaluate.is_anonymous == 1) this.goodsEvaluate.member_name = this.goodsEvaluate
              .member_name.replace(
        ...
        this.$api.sendRequest({
          ...
          success: res => {
            if (res.code == 0) {
              ...
              wxJS.setShareData({
                title: this.goodsSkuDetail.sku_name,
                desc: shareConfig.goods_param_1 + '：' + this.$lang('common.currencySymbol') + this
