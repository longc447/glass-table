<template>
    ...
    <view class="input-wrap" v-if="globalConfig.isChooseKf">
      ...
      <view>
        ...
        <picker mode="selector" :range="site_list" range-key="site_name" @change="handleChangeSite">
          <view :style="{ color: site_item.site_name ? '#000000' : '#929292' }">
            {{ site_item.site_name || '请选择服务客服' }}
          </view>
        </picker>
      </view>
    </view>
  ...
  <view class="shenhe" v-else-if="show == 2">
    <view class="img2">
      <image
        src="https://dafeiapp.oss-cn-beijing.aliyuncs.com/shenheshibai.png?versionId=CAEQExiBgMD9zbfN3hciIDA0ODY0YWMzODE2ODQzMTc5ZTIyYWVjMTc0YjE5YjZl"></image>
    </view>
    ...
    methods: {
      ...
      submitFn() {
        if (!this.store_name) {
          ...
          this.msg('请上传营业执照');
          return;
        } else if (!this.site_item.site_id && this.globalConfig.isChooseKf) {
          this.msg('请选择服务客服');
          return;
        }
