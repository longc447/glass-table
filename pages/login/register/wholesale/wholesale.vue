<template>
  <view class="container" v-if="show == 0">
    <view class="input-wrap">
      ...
      <view><input v-model="store_name" type="text" placeholder="请填写门店名称" /></view>
    </view>
    <view class="input-wrap">
      <view>
        <text>
    ...
    <view class="input-wrap">
      ...
      <view><input v-model="address" type="text" placeholder="请填写地址" /></view>
    </view>
    <view class="input-wrap">
      <view>
        <text>
      ...
      <view><input v-model="realname" type="text" placeholder="请填写联系人" /></view>
    </view>
    <view class="input-wrap">
      <view><text>手机号:</text></view>
      <view><input v-model="mobile" type="number" placeholder="请填写手机号" /></view>
    </view>
    <!-- <view class="input-wrap">
			<view>
    ...
    <view class="input-wrap" v-if="globalConfig.isChooseKf">
      ...
      <view>
        ...
        <picker mode="selector" :range="site_list" range-key="site_name" @change="handleChangeSite">
          <view :style="{ color: site_item.site_name ? '#000000' : '#929292' }">{{ site_item.site_name || '请选择服务客服' }}</view>
        </picker>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    async getWholesale() {
      let { data } = await this.$api.sendRequest({ url: '/api/member/shopList', async: false });
      this.site_list = data;
    ...
    submitFn() {
      ...
      this.$api.sendRequest({
        ...
        data: {
          ...
          site_id: this.site_item.site_id,
        },
        success: res => {
          if (res.code > 0) {
