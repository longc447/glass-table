<template>
	...
	<view class="shenhe" v-else-if="show == 2">
		<view class="img2">
			<image
				src="https://dafeiapp.oss-cn-beijing.aliyuncs.com/shenheshibai.png?versionId=CAEQExiBgMD9zbfN3hciIDA0ODY0YWMzODE2ODQzMTc5ZTIyYWVjMTc0YjE5YjZl">
			</image>
		</view>
	...
	<view class="shenhe" v-else-if="show == 3">
		<view class="img2">
			<svg t="1646400750999" class="icon" viewBox="0 0 1025 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
				p-id="1244" width="100" height="100">
				<path
			...
			async getWholesale() {
				if (!uni.getStorageSync('token')) {
					this.$util.redirectTo('/pages/login/login/login', {
						back: '/pages/login/register/wholesale/wholesale'
					});
				}
				let {
					data
				} = await this.$api.sendRequest({
					url: '/api/member/shopList',
					async: false
				});
				this.site_list = data;
