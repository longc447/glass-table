<template>
  <view :data-theme="themeStyle" class="allPage" :class="{ bottom: isIPhone13?'100px':hasData }">
        ...
        <view class="cart-wrap" v-for="(siteItem, siteIndex) in cartData" :key="siteIndex">
          ...
          <block v-for="(item, cartIndex) in siteItem.cartList" :key="cartIndex">
            <view class="cart-goods" @touchstart="touchS($event)" @touchend="touchE($event, item)">
              <view class="goods-wrap" :class="{ edit: item.edit }">
                ...
                <view @click="toGoodsDetail(item)" class="goods-img">
                  <image
                    ...
                    @error="imgError(siteIndex, cartIndex)"
                ...
                <view class="goods-info">
                  ...
                  <view class="goods-sub-section">
                    ...
                    <uni-number-box
                      :min="item.min_num > 1 ? item.min_num : 1"
              ...
              <view
                class="item-del color-base-bg"
                :class="{ show: item.edit }"
                @click="deleteCart('single', siteIndex, cartIndex)">
                {{ $lang('del') }}
              </view>
            </view>
          </block>
      ...
      <block v-else>
        <view class="cart-empty">
          ...
          <button
            ...
            class="button"
            v-if="token != ''"
            @click="$util.redirectTo('/promotionpages/wholesale/list/list', {}, 'redirectTo')">
            去逛逛
          </button>
...
<script>
  ...
  methods: {
  	isIPhone13() {
   	...
   	// #ifdef MP
   	if (systemInfo.model.search('iPhone 13') != -1 || systemInfo.model.search('iPhone 14') != -1) {
   		isIphone11 = true;
   	}
   	// #endif
   	return isIphone11;
